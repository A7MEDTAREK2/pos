...
import '../widget/driver_report_table.dart';

// استبدل بمسارات الكيوبت والستيت الخاصة بالمناديب لديك
...
class DriverReportScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: BlocBuilder<DriverReportCubit, DriverReportState>(
  ...
  Widget _buildContent(BuildContext context, DriverReportState state) {
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            TextButton.icon(
              onPressed: () => context.read<DriverReportCubit>().loadReport(),
              icon: Icon(Icons.refresh, size: 18, color: colorScheme.primary),
              label: Text('إعادة المحاولة', style: theme.textTheme.labelLarge?.copyWith(color: colorScheme.primary)),
            ),
          ],
        ),
      ),
    ...
    return Container(
      ...
      child: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.delivery_dining_outlined, size: 64, color: colorScheme.onSurface.withOpacity(0.2)),
            const SizedBox(height: 16),
            Text(
            ...
            Text(
              'لم يتم تسجيل أي طلبات توصيل حتى الآن',
